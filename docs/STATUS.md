...
- **BLOQUEADOR P1 CONFIRMADO:** os scripts de captura/ingestão de sessão ainda são os de 2026-07-30 e esperam o layout legado `agents/main/sessions/sessions.json` + JSONL. O OpenClaw 2026.9.5 usa SQLite canônico em `agents/<agentId>/agent/openclaw-agent.sqlite`; o inventário falhou exatamente porque `sessions.json` não existe. `mimir-ingest-session.py` continua aceitando somente `--dry-run`.
- **PRÓXIMO:** inventariar o store SQLite via CLI/API somente leitura, inspecionar a única linha relevante do root crontab e projetar uma captura compatível com OpenClaw 2026.9.5; não recriar arquivos legados nem escrever na produção.
