...
- **PRÓXIMO:** testar estruturalmente e sem exibir conteúdo a API suportada `chat.history` para verificar owner marker, IDs/seq, paginação e sinais de truncamento/redaction; se suficiente, substituir o coletor legado por `sessions --json` + `chat.history`, sem acesso SQL direto ao store do OpenClaw.
